...
const IndividualServices = () => {
  ...
  return (
    <div className="bg-[#e9ecef] px-[1rem] sm:px-[2rem] md:px-[3rem] lg:px-[4rem] xl:px-[5rem] py-[5rem] flex flex-col items-center gap-[2rem]">
      <div className="grid xl:grid-cols-2 rounded-2xl">
        <div className="px-[2rem] sm:px-[3rem] md:px-[5rem] lg:px-[8rem] py-[5rem] flex flex-col gap-[3rem]">
          <span className="font-['rubik_doodle_shadow'] text-black text-6xl font-bold">{data?.title.heading}</span>
          <div className="flex flex-col gap-[1rem]">
            <p className="text-black">{data?.sub}</p>
          </div>
        </div>
        <div className={`${data?.imgClass} rounded-full`}></div>
      </div>
